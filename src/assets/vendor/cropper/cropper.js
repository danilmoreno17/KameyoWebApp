...
(function (global, factory) {
  ...
  var IS_SAFARI_OR_UIWEBVIEW =
    navigator && REGEXP_USERAGENT.test(navigator.userAgent);
  ...
  var render$1 = {
        ...
        if (viewMode) {
          if (viewMode > 1) {
            ...
            minCanvasWidth = Math.max(
              minCanvasWidth,
              cropped ? cropBox.width : 0
            );
          } else if (minCanvasHeight) {
            minCanvasHeight = Math.max(
              minCanvasHeight,
              cropped ? cropBox.height : 0
            );
          } else if (cropped) {
    ...
    renderCanvas: function renderCanvas(isChanged) {
      ...
      canvas.width = Math.min(
        Math.max(canvas.width, canvas.minWidth),
        canvas.maxWidth
      );
      canvas.height = Math.min(
        Math.max(canvas.height, canvas.minHeight),
        canvas.maxHeight
      );

      self.limitCanvas(false, true);

      canvas.oldLeft = canvas.left = Math.min(
        Math.max(canvas.left, canvas.minLeft),
        canvas.maxLeft
      );
      canvas.oldTop = canvas.top = Math.min(
        Math.max(canvas.top, canvas.minTop),
        canvas.maxTop
      );
    ...
    initCropBox: function initCropBox() {
      ...
      cropBox.width = Math.min(
        Math.max(cropBox.width, cropBox.minWidth),
        cropBox.maxWidth
      );
      cropBox.height = Math.min(
        Math.max(cropBox.height, cropBox.minHeight),
        cropBox.maxHeight
      );
    ...
    limitCropBox: function limitCropBox(isSizeLimited, isPositionLimited) {
      ...
      if (isSizeLimited) {
        ...
        var maxCropBoxWidth = Math.min(
          containerWidth,
          limited ? canvas.width : containerWidth
        );
        var maxCropBoxHeight = Math.min(
          containerHeight,
          limited ? canvas.height : containerHeight
        );
      ...
      if (isPositionLimited) {
        if (limited) {
          ...
          cropBox.maxLeft =
            Math.min(containerWidth, canvas.left + canvas.width) -
            cropBox.width;
          cropBox.maxTop =
            Math.min(containerHeight, canvas.top + canvas.height) -
            cropBox.height;
        } else {
          cropBox.minLeft = 0;
          cropBox.minTop = 0;
  ...
  var EVENT_MOUSE_UP = PointerEvent
  ...
  var events = {
    bind: function bind() {
      ...
      $(document)
        .on(EVENT_MOUSE_MOVE, (self.onCropMove = proxy(self.cropMove, this)))
        .on(EVENT_MOUSE_UP, (self.onCropEnd = proxy(self.cropEnd, this)));
  ...
  var handlers = {
    ...
    dblclick: function dblclick() {
      ...
      self.setDragMode(
        self.$dragBox.hasClass("cropper-crop") ? "move" : "crop"
      );
    },
    ...
    cropEnd: function cropEnd(e) {
      ...
      if (self.cropping) {
        self.cropping = false;
        self.$dragBox.toggleClass(
          "cropper-modal",
          self.cropped && self.options.modal
        );
      }
  ...
  var change$1 = {
    change: function change(e) {
      ...
      if (self.limited) {
        ...
        maxWidth =
          minLeft +
          Math.min(container.width, canvas.width, canvas.left + canvas.width);
        maxHeight =
          minTop +
          Math.min(container.height, canvas.height, canvas.top + canvas.height);
      }
  ...
  var methods = {
    ...
    move: function move(offsetX, offsetY) {
      ...
      self.moveTo(
        isUndefined(offsetX) ? offsetX : canvas.left + Number(offsetX),
        isUndefined(offsetY) ? offsetY : canvas.top + Number(offsetY)
      );
    },
    ...
    zoomTo: function zoomTo(ratio, _event) {
      ...
      if (ratio >= 0 && self.ready && !self.disabled && options.zoomable) {
        ...
        if (originalEvent) {
          ...
          var center =
            pointers && objectKeys(pointers).length
    ...
    getCroppedCanvas: function getCroppedCanvas(options) {
      ...
      var parameters = (function () {
        ...
        var srcX =
          data.x +
          (canvasData.naturalWidth * (Math.abs(data.scaleX || 1) - 1)) / 2;
        var srcY =
          data.y +
          (canvasData.naturalHeight * (Math.abs(data.scaleY || 1) - 1)) / 2;
        var srcWidth = void 0;
      ...
      [
        ...
        },
        {
          ...
          value: function load(url) {
            ...
            xhr.withCredentials =
              $this.prop("crossOrigin") === "use-credentials";
            xhr.send();
          },
        },
        {
        ...
        },
        {
          ...
          value: function clone() {
            ...
            if (self.isImg) {
              ...
              $clone
                .one("load", $.proxy(self.start, this))
        ...
        },
        {
          ...
          value: function build() {
            ...
            if (!options.cropBoxResizable) {
              $cropBox
                .find(".cropper-line, .cropper-point")
