...
(function ($) {
  ...
  Progressbar.prototype.transition = function () {
    ...
    var update =
      options.update && typeof options.update === "function"
        ? options.update
        : Progressbar.defaults.update;
    var done =
      options.done && typeof options.done === "function"
        ? options.done
    ...
    var percentage = Math.round(
      (100 * (data_transitiongoal - aria_valuemin)) /
        (aria_valuemax - aria_valuemin)
    );

    if (options.display_text === "center" && !$back_text && !$front_text) {
      this.$back_text = $back_text = $("<span>")
        .addClass("progressbar-back-text")
        .prependTo($parent);
      this.$front_text = $front_text = $("<span>")
    ...
    setTimeout(function () {
      ...
      var progress = setInterval(function () {
        ...
        current_value = Math.round(
          aria_valuemin +
            (this_size / parent_size) * (aria_valuemax - aria_valuemin)
        );
        ...
        if (options.display_text !== "none") {
          text = options.use_percentage
            ? options.percent_format(current_percentage)
