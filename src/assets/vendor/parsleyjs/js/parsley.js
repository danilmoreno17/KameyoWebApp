...
(function (global, factory) {
  ...
  Base.prototype = {
    ...
    _pipeAccordingToValidationResult:
      function _pipeAccordingToValidationResult() {
        var _this = this;
  ...
  var ValidatorRegistry__parseArguments =
    function ValidatorRegistry__parseArguments(type, args) {
      return args.map(Utils.parse[type]);
    };
  // operatorToValidator returns a validating function for an operator function, applied to the given type
  var ValidatorRegistry__operatorToValidator =
    function ValidatorRegistry__operatorToValidator(type, operator) {
      return function (value) {
        for (
          var _len = arguments.length,
            requirementsAndInput = Array(_len > 1 ? _len - 1 : 0),
            _key = 1;
          _key < _len;
          _key++
        ) {
        ...
        return operator.apply(
          ...
          [value].concat(
            _toConsumableArray(
              ValidatorRegistry__parseArguments(type, requirementsAndInput)
            )
          )
        );
      };
  ...
  var ValidatorRegistry__comparisonOperator =
    function ValidatorRegistry__comparisonOperator(operator) {
      return {
        validateDate: ValidatorRegistry__operatorToValidator("date", operator),
        validateNumber: ValidatorRegistry__operatorToValidator(
          "number",
          operator
        ),
  ...
  Form.prototype = {
    ...
    whenValidate: function whenValidate() {
      ...
      return (_Utils$all$done$fail$always = Utils.all(promises)
        .done(function () {
    ...
    _bindFields: function _bindFields() {
      ...
      this._withoutReactualizingFormOptions(function () {
        _this8.$element
          .find(_this8.options.inputs)
          .not(_this8.options.excluded)
  ...
  Constraint.prototype = {
    validate: function validate(value, instance) {
      ...
      return (_validator = this.validator).validate.apply(
        _validator,
  ...
  Field.prototype = {
    ...
    whenValidate: function whenValidate() {
      ...
      return (_whenValid$always$done$fail$always = this.whenValid({
        ...
        _whenValid$always$done$fail$always,
        _toConsumableArray(this._pipeAccordingToValidationResult())
      );
    },
    ...
    _validateConstraint: function _validateConstraint(value, constraint) {
      ...
      return Utils.all([result]).fail(function (errorMessage) {
        if (!(_this12.validationResult instanceof Array))
          _this12.validationResult = [];
      ...
      if (name) {
        $('input[name="' + name + '"]').each(function (i, input) {
          if (input.type === "radio" || input.type === "checkbox")
            input.setAttribute(
              _this13.options.namespace + "multiple",
              _this13.options.multiple
            );
        });
      }
      ...
      behavesOk: function behavesOk(evt) {
        if (_this14.isNativeEvent(evt)) {
          $(document) // Simply unbinds the testing handler
            .off("input.inputevent", evt.data.selector, _this14.behavesOk)
            .off("change.inputevent", evt.data.selector, _this14.misbehaves);
        }
      },
