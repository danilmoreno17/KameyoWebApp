...
(function () {
          ...
          ? ((d = a.showNotification(c.embeddingInProgress, "info")),
            h.loadContent(b, {
              ...
              callback: function () {
                var b = a
                  .editable()
                  .findOne('a[data-cke-autoembed\x3d"' + g + '"]');
                if (b) {
  ...
  CKEDITOR.plugins.add("autoembed", {
    ...
    lang: "cs,de,de-ch,en,eo,eu,fr,it,km,ko,ku,mk,nb,pl,pt,pt-br,ru,tr,ug,uk,zh,zh-cn",
    init: function (a) {
      var g = 1,
        b;
      a.on(
        ...
        function (c) {
          if (
            c.data.dataTransfer.getTransferType(a) ==
            ...
            if ((b = null != d && decodeURI(d[1]) == decodeURI(d[2])))
              c.data.dataValue =
                '\x3ca data-cke-autoembed\x3d"' +
                ++g +
                '"' +
                c.data.dataValue.substr(2);
          }
        },
        null,
        null,
