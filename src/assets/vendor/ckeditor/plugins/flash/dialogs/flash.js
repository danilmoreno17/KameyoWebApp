...
(function () {
  ...
  function c(a, b, c) {
    ...
    if (h)
      for (
        var f = "" === this.getValue(),
          e = this instanceof CKEDITOR.ui.dialog.checkbox,
          d = 0;
        d < h.length;
        d++
      ) {
        ...
        switch (g.type) {
          case 1:
            if (!a || ("data" == g.name && b && !a.hasAttribute("data")))
              continue;
            var m = this.getValue();
            f || (e && m === g["default"])
              ? a.removeAttribute(g.name)
              : a.setAttribute(g.name, m);
            break;
          case 2:
            if (!a) continue;
            m = this.getValue();
  ...
  CKEDITOR.dialog.add("flash", function (a) {
    var l = !a.config.flashEmbedTagOnly,
      ...
      f =
        "\x3cdiv\x3e" +
        CKEDITOR.tools.htmlEncode(a.lang.common.preview) +
        '\x3cbr\x3e\x3cdiv id\x3d"cke_FlashPreviewLoader' +
        CKEDITOR.tools.getNextNumber() +
        '" style\x3d"display:none"\x3e\x3cdiv class\x3d"loading"\x3e\x26nbsp;\x3c/div\x3e\x3c/div\x3e\x3cdiv id\x3d"cke_FlashPreviewBox' +
        CKEDITOR.tools.getNextNumber() +
        '" class\x3d"FlashPreviewBox"\x3e\x3c/div\x3e\x3c/div\x3e';
    return {
