...
CKEDITOR.plugins.add("notification", {
  lang: "cs,da,de,de-ch,en,eo,eu,fr,gl,id,it,km,ko,ku,nb,nl,pl,pt,pt-br,ru,sv,tr,ug,uk,zh,zh-cn",
  requires: "toolbar",
  init: function (b) {
    function a(b) {
    ...
    b.on("key", function (c) {
      if (27 == c.data.keyCode) {
        ...
        d.length &&
          (a(b.lang.notification.closed), d[d.length - 1].hide(), c.cancel());
      }
    });
  },
});
...
Area.prototype = {
  ...
  _layout: function () {
    function b() {
      a.setStyle("left", k(n + d.width - f - h));
    }
    var a = this.element,
      c = this.editor,
    ...
    (f && h) ||
      ((l = this.element.getChild(0)),
      (f = this._notificationWidth = l.getClientRect().width),
      (h = this._notificationMargin =
        parseInt(l.getComputedStyle("margin-left"), 10) +
        parseInt(l.getComputedStyle("margin-right"), 10)));
    c.isVisible() && g.bottom > d.top && g.bottom < d.bottom - m.height
      ? a.setStyles({ position: "fixed", top: k(g.bottom) })
      : 0 < d.top
      ? a.setStyles({ position: "absolute", top: k(e.y) })
      : e.y + d.height - m.height > p.y
      ? a.setStyles({ position: "fixed", top: 0 })
      : a.setStyles({
          position: "absolute",
          top: k(e.y + d.height - m.height),
        });
    var n =
      "fixed" == a.getStyle("position")
        ? d.left
        : "static" != r.getComputedStyle("position")
        ? e.x - t.x
        : e.x;
    d.width < f + h
      ? e.x + f + h > p.x + q.width
      ...
      : e.x + f + h > p.x + q.width
      ? a.setStyle("left", k(n))
      : e.x + d.width / 2 + f / 2 + h > p.x + q.width
      ? a.setStyle("left", k(n - e.x + p.x + q.width - f - h))
      : 0 > d.left + d.width - f - h
      ? b()
      : 0 > d.left + d.width / 2 - f / 2
      ? a.setStyle("left", k(n - e.x + p.x))
      : a.setStyle("left", k(n + d.width / 2 - f / 2 - h / 2));
  },
};
