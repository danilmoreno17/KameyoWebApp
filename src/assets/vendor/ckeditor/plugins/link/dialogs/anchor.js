...
CKEDITOR.dialog.add("anchor", function (c) {
  function d(a, b) {
    return a.createFakeElement(
      a.document.createElement("a", { attributes: b }),
      "cke_anchor",
      "anchor"
    );
  }
  return {
    ...
    onOk: function () {
      ...
      if (this._.selectedElement)
        this._.selectedElement.data("cke-realelement")
          ? ((a = d(c, a)),
            a.replace(this._.selectedElement),
            CKEDITOR.env.ie && c.getSelection().selectElement(a))
          : this._.selectedElement.setAttributes(a);
      else {
        var b = c.getSelection(),
          b = b && b.getRanges()[0];
        b.collapsed
          ? ((a = d(c, a)), b.insertNode(a))
          : (CKEDITOR.env.ie &&
    ...
    onShow: function () {
      var a = c.getSelection(),
        ...
        d = b && b.data("cke-realelement"),
        e = d
          ? CKEDITOR.plugins.link.tryRestoreFakeAnchor(c, b)
          : CKEDITOR.plugins.link.getSelectedLink(c);
      if (e) {
        this._.selectedElement = e;
        var f = e.data("cke-saved-name");
        this.setValueOf("info", "txtName", f || "");
        !d && a.selectElement(e);
        b && (this._.selectedElement = b);
      }
