...
(function () {
  ...
  g.prototype = {
    exec: function (a) {
      ...
      if (c) {
        for (
          ...
          for (
            g = d[m].createIterator(), g.enlargeBr = b != CKEDITOR.ENTER_BR;
            (f = g.getNextParagraph(b == CKEDITOR.ENTER_P ? "p" : "div"));
            ...
            if (!f.isReadOnly()) {
              ...
              var l =
                  e &&
                  (f.$.className = CKEDITOR.tools.ltrim(
                    f.$.className.replace(this.cssClassRegex, "")
                  )),
                p =
              ...
              e
                ? p
                  ? f.addClass(e)
                  : l || f.removeAttribute("class")
                : p && f.setStyle("text-align", this.value);
            }
        a.focus();
    ...
    refresh: function (a, c) {
      ...
      this.setState(
        "body" != b.getName() &&
          n(b, this.editor.config.useComputedState) == this.value
