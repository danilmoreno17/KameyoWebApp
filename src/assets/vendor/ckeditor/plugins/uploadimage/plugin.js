  ...
  CKEDITOR.plugins.add("uploadimage", {
    ...
    init: function (d) {
      if (CKEDITOR.plugins.clipboard.isFileApiSupported) {
        ...
        h
          ? (f.addUploadWidget(d, "uploadimage", {
              ...
              onUploaded: function (a) {
                this.replaceWith(
                  '\x3cimg src\x3d"' +
                    a.url +
                    '" width\x3d"' +
                    this.parts.img.$.naturalWidth +
                    '" height\x3d"' +
                    this.parts.img.$.naturalHeight +
                    '"\x3e'
                );
              },
            }),
            d.on("paste", function (a) {
              if (a.data.dataValue.match(/<img[\s\S]+data:/i)) {
                ...
                var c = document.implementation.createHTMLDocument(""),
                  c = new CKEDITOR.dom.element(c.body),
                  l,
                  b,
                  g;
                ...
                for (g = 0; g < l.count(); g++) {
                  ...
                  var e =
                      b.getAttribute("src") &&
                      "data:" == b.getAttribute("src").substring(0, 5),
                    k = null === b.data("cke-realelement");
                  e &&
                    k &&
                    !b.data("cke-upload-id") &&
                    !b.isReadOnly(1) &&
                    ((e = d.uploadRepository.create(b.getAttribute("src"))),
                    e.upload(h),
                    f.markElement(b, "uploadimage", e.id),
                    f.bindNotifications(d, e));
                }
