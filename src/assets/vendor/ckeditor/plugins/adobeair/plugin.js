          ...
          for (var b = 0; b < l.length; b++)
            (function (b) {
              ...
              a.hasAttribute("on" + b) &&
                ...
                a.on(b, function (b) {
                  var c =
                      /(return\s*)?CKEDITOR\.tools\.callFunction\(([^)]+)\)/.exec(
                        d
                      ),
                    k = c && c[1],
                    e = c && c[2].split(","),
                  ...
                  if (e) {
                    for (var m = e.length, h, g = 0; g < m; g++) {
                      ...
                      var f = h.match(/^(["'])([^"']*?)\1$/);
                      if (f) e[g] = f[2];
                      else if (h.match(/\d+/)) e[g] = parseInt(h, 10);
                      else
                        switch (h) {
                          case "this":
                            e[g] = a.$;
  ...
  CKEDITOR.plugins.add("adobeair", {
    onLoad: function () {
      CKEDITOR.env.air &&
        ((CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(
          ...
          function (a) {
            function c(b, a, c, k) {
              a = b.append(a);
              (c =
                CKEDITOR.htmlParser.fragment.fromHtml(c).children[0]
                  .attributes) && a.setAttributes(c);
              ...
              if (this.getBody()) {
                ...
                b.replace(
                  ...
                  function (b, a, c) {
                    d.getBody().setHtml(c);
                    (b =
                      CKEDITOR.htmlParser.fragment.fromHtml(a).children[0]
                        .attributes) && d.getBody().setAttributes(b);
                  }
                );
        ...
        CKEDITOR.ui.on("ready", function (a) {
          a = a.data;
          if (a._.panel) {
