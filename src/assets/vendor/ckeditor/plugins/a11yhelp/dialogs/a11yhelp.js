...
CKEDITOR.dialog.add("a11yHelp", function (l) {
  ...
  var f = [CKEDITOR.ALT, CKEDITOR.SHIFT, CKEDITOR.CTRL],
    p = /\$\{(.*?)\}/g,
    t = (function () {
      var a = l.keystrokeHandler.keystrokes,
        g = {},
        c;
      for (c in a) g[a[c]] = c;
      return function (a, c) {
        ...
        if (g[c]) {
          b = g[c];
          for (var h, k, m = [], d = 0; d < f.length; d++)
            (k = f[d]),
              (h = b / f[d]),
              1 < h && 2 >= h && ((b -= k), m.push(e[k]));
  ...
  return {
    ...
    contents: [
      {
        id: "info",
        label: l.lang.common.generalTab,
        expand: !0,
        elements: [
          {
            type: "html",
            ...
            html:
              (function () {
                for (
                  ...
                  for (
                    var b = c[f], h = [], k = b.items, m = k.length, d = 0;
                    d < m;
                    d++
                  ) {
                    var q = k[d],
                      r = q.legend.replace(p, t);
                    r.match(p) ||
                      h.push(
