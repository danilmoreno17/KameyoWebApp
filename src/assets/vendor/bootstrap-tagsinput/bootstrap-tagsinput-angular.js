angular.module("bootstrap-tagsinput", []).directive("bootstrapTagsinput", [
  function () {
    ...
    return {
      ...
      link: function (scope, element, attrs) {
        $(function () {
          ...
          select.tagsinput(
            scope.$parent[attrs.options || ""] || {
              typeahead: {
                source: angular.isFunction(typeaheadSource)
                  ? typeaheadSource
                  : null,
              },
              itemValue: getItemProperty(scope, attrs.itemvalue),
              itemText: getItemProperty(scope, attrs.itemtext),
              confirmKeys: getItemProperty(scope, attrs.confirmkeys)
                ? JSON.parse(attrs.confirmkeys)
                : [13],
          ...
          select.on("itemRemoved", function (event) {
            ...
            if (idx !== -1) scope.model.splice(idx, 1);
          });
